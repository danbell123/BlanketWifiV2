...
const TimelineItem = React.forwardRef<HTMLDivElement, { activity: Activity }>(({ activity }, ref) => {
  ...
  return (
    <div className="flex items-center mb-6" ref={ref}>
      <div className="flex-shrink-0 w-12 h-12 flex items-center justify-center rounded-full mr-4 z-10"
      ...
      <div className="flex flex-row bg-card p-4 rounded-lg shadow w-full">
        <div className='w-full flex flex-col gap-2 justify-end'>
          ...
          <time className="block text-sm text-popover-foreground">{new Date(activity.timestamp).toLocaleString()}</time>
          <p>{activity.description}</p>
        </div>
        <div className='flex flex-col justify-end'>
          {activity.type === 'pulseSent' && <Button variant={"outline"} size={"sm"}>View Pulse</Button>}
          {activity.type === 'interaction' && <Button variant={"outline"} size={"sm"}>View Pulse</Button>}
        </div>
      </div>
    </div>
  );
